...
export default function AIInsights({ transactions = [], budgets = [] }) {
  ...
  const generateInsights = () => {
    ...
    const categories = transactions
      .filter(t => t.type === 'expense' && t.date && t.date.startsWith(currentMonth))
      .reduce((acc, curr) => {
        acc[curr.category] = (acc[curr.category] || 0) + Math.abs(curr.amount || 0);
  ...
  // Safe access for current insight
  const currentInsight = insights[currentIndex] || insights[0] || { id: 'default', title: 'Loading...', description: '', icon: Sparkles, color: 'text-gray-400', bg: 'bg-gray-500/10' };
