  ...
  const fetchData = async () => {
    ...
    try {
      ...
      try {
        ...
        addToast('Could not load transactions. Make sure the table exists.', 'error');
      }

      // Fetch budgets
      try {
        const budgetData = await budgetService.getBudgets(currentMonth);
        setBudgets(budgetData || []);
      } catch (err) {
        console.error('Budgets fetch error:', err);
        // Silently fail budgets or show minor warning, don't crash the whole dashboard
      }
    } catch (error) {
